...
let test_asm_cfg _ =
  ...
  let v = [start; node 0 (je (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (je (lab "tru")));
           (node 0 (je (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit);
           (node 0 (je (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
  ...
  let v = [start; node 0 (jne (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jne (lab "tru")));
           (node 0 (jne (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jne (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jnz (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jnz (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jnz (lab "tru")));
           (node 0 (jnz (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jnz (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jz (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jz (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jz (lab "tru")));
           (node 0 (jz (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jz (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jg (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jg (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jg (lab "tru")));
           (node 0 (jg (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jg (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jge (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jge (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jge (lab "tru")));
           (node 0 (jge (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jge (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jl (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jl (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jl (lab "tru")));
           (node 0 (jl (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jl (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jle (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jle (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jle (lab "tru")));
           (node 0 (jle (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jle (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [js (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (js (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (js (lab "tru")));
           (node 0 (js (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (js (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jns (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jns (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jns (lab "tru")));
           (node 0 (jns (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jns (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jc (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jc (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jc (lab "tru")));
           (node 0 (jc (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jc (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;

  let asms = [jnc (lab "tru"); ret; label_op "tru"; ret] in
  let v = [start; node 0 (jnc (lab "tru")); node 1 ret;
           node 2 (label_op "tru"); node 3 ret; exit] in
  let e = [(start, norm, node 0 (jnc (lab "tru")));
           (node 0 (jnc (lab "tru")), norm, node 2 (label_op "tru"));
           (node 2 (label_op "tru"), norm, node 3 ret);
           (node 3 ret, norm, exit);
           (node 0 (jnc (lab "tru")), norm, node 1 ret);
           (node 1 ret, norm, exit)] in
  make_graph v e === Cfg.AsmCfg.create_cfg asms;
